...
class KekeWitkeySpace extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['username'], 'required'],
            [['group_id', 'isvip', 'status', 'indus_id', 'indus_pid', 'reg_time', 'credit', 'balance_status', 'pub_num', 'take_num', 'nominate_num', 'accepted_num', 'vip_start_time', 'vip_end_time', 'score', 'buyer_credit', 'buyer_good_num', 'buyer_total_num', 'seller_credit', 'seller_good_num', 'seller_total_num', 'studio_id', 'last_login_time', 'views', 'goldenfarm_level', 'spread_num', 'university', 'focus_num', 'goldenfarm_start', 'goldenfarm_end', 'auth_bank', 'auth_email', 'auth_enterprise', 'auth_mobile', 'auth_realname', 'auth_university', 'pre_type', 'auth_design', 'entscale'], 'integer'],
    ...
    public static function getUidByUsername($username)
    {
        ...
        $uc_keke_uid = yii::$app->redis->GET('UC_KEKE_UID_' . md5($username));
        if (!$uc_keke_uid)
        {
            $uc_keke_uid = self::find()
                ->select('uid')
                ->where(['username' => $username])
                ->scalar();
            yii::$app->redis->SET('UC_KEKE_UID_' . md5($username), $uc_keke_uid);
        }
        return $uc_keke_uid;
    }
}
